...
// Presencia de habitos de consumo externos.
...
theta  = 1/(1+(1-lab_ss)/lab_ss*alpha/((1-betta+alpha*betta*delta)/(1-betta+betta*delta)-G_Y)*1/(1-phi));
...
model;
theta/(exp(c)-phi*exp(c(-1))) = (1-theta)/((1-exp(lab))*exp(w));
theta/(exp(c)-phi*exp(c(-1))) = betta*(theta/(exp(c(+1))-phi*exp(c)))*(1+exp(r(+1)));
