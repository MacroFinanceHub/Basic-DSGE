...
// Se incluyen costos de ajuste a la inversión 
...
exp(q)/exp(c)=betta*1/exp(c(+1))*(delta+exp(r(+1))+exp(q(+1))*(1-delta));
exp(w)       =alpha*exp(y)/exp(lab);
exp(r)+delta =(1-alpha)*exp(y)/exp(kap);
exp(y)       =exp(c)+exp(innv)+exp(g);
exp(kap(+1)) =(1-delta)*exp(kap)+(1-psi/2*(exp(innv)/exp(innv(-1))-1)^2)*exp(innv);
exp(y)       =exp(z)*exp(kap)^(1-alpha)*exp(lab)^alpha;
exp(q)       =1+exp(q)*psi/2*(exp(innv)/exp(innv(-1))-1)^2+exp(q)*psi*(exp(innv)/exp(innv(-1))-1)*(exp(innv)/exp(innv(-1)))-betta*exp(c)/exp(c(+1))*exp(q(+1))*psi*(exp(innv(+1))/exp(innv)-1)*(exp(innv(+1))/exp(innv))^2;
