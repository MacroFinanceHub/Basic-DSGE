...
// TAREA 1: INTRODUCIR CHOQUES DE PREFERENCIAS Y DE OFERTA LABORAL
// (c) Carlos Rojas Quiroz 

var lab c w r y kap innv z g lambda gamma;
predetermined_variables kap;
varexo e_z e_g e_lambda e_gamma;
parameters alpha delta betta theta rho_z rho_g rho_lambda rho_gamma
...
model;
theta*exp(lambda)/exp(c) =(1-theta)*exp(gamma)/((1-exp(lab))*exp(w));
exp(lambda)/exp(c)     =betta*exp(lambda(+1))/exp(c(+1))*(1+exp(r(+1)));
...
lambda       =(1-rho_lambda)*log(lambda_ss) + rho_lambda*lambda(-1) + e_lambda;
gamma        =(1-rho_gamma)*log(gamma_ss) + rho_gamma*gamma(-1) + e_gamma;
end;
